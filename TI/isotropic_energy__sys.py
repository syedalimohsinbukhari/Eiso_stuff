...
redshift, start, end = [float(i) for i in sys.argv[2:-1]]

z_s = f'0{int(redshift)}' if 1 <= redshift < 10 else str(int(redshift)) if redshift == 10 else str(redshift)
...
output_directory = f'{cwd}/output__{z_s}/'
...
np.save(f'{output_directory}/parameters_{model_name}__{start}_{end}__{z_s}', mvd_)
...
arr1 = functions.parameter_plotting(mv_, len(par_n[:-2]), par_n[:-2], n_bins, sigma)
plt.savefig(f'{output_directory}/{model_name.lower()}_parameters__{start}_{end}__{z_s}.pdf')
plt.close()

arr2 = functions.parameter_plotting(mv_, len(par_n[-2:]), par_n[-2:], n_bins, sigma)
plt.savefig(f'{output_directory}/{model_name.lower()}_isotropic_energy__{start}_{end}__{z_s}.pdf')
plt.close()

functions.save_val_err(arr1, arr2, output_directory, model_name.lower(), f'__{start}_{end}__{z_s}')

_save = pd.DataFrame(mv_)

np.save(f'{output_directory}/{model_name.lower()}_isotropic_energy_details', _save.to_numpy())
