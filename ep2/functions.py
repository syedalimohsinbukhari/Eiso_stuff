...
from spectral_models import band_n, sbpl_n, cpl_n
...
class IsotropicEnergy:
    ...
    def isotropic_energy(self, energy, pars):
        if self.model_name == 'band':
            _fluence = band_n(energy, pars[0:4])
        elif self.model_name == 'sbpl':
            _fluence = sbpl_n(energy, pars[0:6])
        elif self.model_name == 'cpl':
            _fluence = cpl_n(energy, pars[0:4])
        else:
            _fluence = 0
...
def save_val_err(arr1, arr2, path, model_name, suffix):
    ...
    t3.to_csv(f'{path}/{model_name.lower()}__parameter_value_err{suffix}.csv', index=False)
